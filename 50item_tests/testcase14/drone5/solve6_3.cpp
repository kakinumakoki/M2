...
double endTemp=0;
long annealing_endTime=90.0,annealing_nowTime;
...
void annealing_insert_search(vector<vector<int>>A)
{
        ...
        double probability=exp((best_score-copy_score)/nowTemp);
        if(copy_score<best_score) annealing_finish_counter=0;
...
double decide_tmp(vector<vector<int>>A,double Tmp){
    ...
    while(1){
        if(solve_counter==loop){
            //cout<<(double)update_num/loop<<endl;
            if(abs((double)update_num/loop-p)<0.01) {
        ...
        if(select<50)//swap
        {
            ...
            swap(copy_answer[x][y],copy_answer[z][w]);
            double copy_score=cal_score_dronenextPoint_test(copy_answer); 
            double probability=exp((best_score-copy_score)/Tmp);
            if(copy_score<best_score||probability>(double)(rand()%RAND_MAX)/RAND_MAX) update_num++;
        }
        else//shift
        {
            int x;
...
void output_txt()
{
    ofstream outputfile("result_6.txt");
    for(int i=0;i<Q-1;i++){
...
int main()
{
    ...
    nowTemp=decide_tmp(solution_where_todeliver,best_score/2);
    start=clock();
    long timer=clock();
    annealing_finish_counter=0;
    while(annealing_finish_counter<10000&&(double)(finish-start)/CLOCKS_PER_SEC<annealing_endTime){
